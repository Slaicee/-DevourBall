...
namespace Slime
{
    ...
    public static class PBF_Utils
    {
        ...
        public const int BubblesCount = 1024;
    ...
    public static class Simulation_PBF
    {
        ...
        public struct ApplyForceJob : IJobParallelFor
        {
            ...
            public void Execute(int i)
            {
                ...
                var velocity = Velocity[i] * 0.95f + Gravity * PBF_Utils.DeltaTime;
                if (p.ID >= 0 && p.ID < Controllers.Length)
                {
                    ParticleController cl = Controllers[p.ID];
                    float3 toCenter = p.Position - cl.Center;

                    // Ellipsoid radii: Y is flatter for a ground-blob look
                    float rxz = math.max(0.1f, cl.Radius);
                    float ry = math.max(0.1f, cl.Radius * 0.75f);
                    float3 radii = new float3(rxz, ry, rxz);
                    float3 normDist = toCenter / radii;
                    float eDist = math.length(normDist);

                    // Hard clamp to ellipsoid surface
                    if (eDist > 1f)
                    {
                        p.Position = cl.Center + toCenter / eDist;
                        toCenter = p.Position - cl.Center;
                        normDist = toCenter / radii;
                        eDist = math.length(normDist);
                    }

                    // Velocity sync: nearly full follow at edges, some freedom at center
                    float follow = 1f - math.saturate(eDist * 0.4f);
                    velocity = math.lerp(velocity, cl.Velocity, follow);

                    // Attraction toward center, stronger near surface
                    float attract = cl.Concentration * 0.4f * math.saturate(eDist * eDist);
                    velocity += attract * PBF_Utils.DeltaTime * math.normalizesafe(-toCenter);
                }

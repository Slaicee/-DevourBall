...
namespace Slime
{
    public class Slime_PBF : MonoBehaviour
    {
        ...
        [SerializeField, Range(0.1f, 100)] private float concentration = 20f;
        [SerializeField, Range(-10, 10)] private float gravity = -2f;
        ...
        [Header("External controller (set by SlimePlayerBridge)")]
        public bool useExternalController;
        public Vector3 externalControllerPos;
        public float externalControllerRadius = 1f;
        public float externalControllerConcentration = 10f;
        ...
        void Start()
        {
            float3 startOffset = float3.zero;
            if (trans != null)
                startOffset = (float3)trans.position * PBF_Utils.InvScale;
            ...
            for (int k = 0; k < PBF_Utils.Width; k++)
            {
                ...
                {
                    Position = new float3(k - half, j, i - half) * 0.5f + startOffset,
                    ID = 0,
                };
            }
        ...
        void Update()
        {
            ...
            {
                if (particleMesh != null && particleMat != null)
        ...
        private void Control()
        {
            ...
            if (useExternalController)
            {
                float3 center = (float3)externalControllerPos * PBF_Utils.InvScale;
                if (trans != null)
                {
                    var rb = trans.GetComponent<Rigidbody>();
                    if (rb != null) _velocity = rb.velocity;
                }
                _controllerBuffer.Add(new ParticleController()
                {
                    Center = center,
                    Radius = externalControllerRadius * PBF_Utils.InvScale,
                    Velocity = (float3)_velocity * PBF_Utils.InvScale,
                    Concentration = externalControllerConcentration,
                });
            }
            else
            {
